...
func _fixed_process(delta):
	...
	if self.is_network_master():
		m_pos = get_global_mouse_pos()
		rset_unreliable("slave_mouse_position", m_pos)
		if Input.is_action_just_pressed("move_to"):
			jump["destinations"].append(m_pos)
			spawn_click_indicator(mouse_pos, "move_to")
		if Input.is_action_just_pressed("attack"):
			atk_loc = m_pos

	var focus = atk_loc if is_state(BUSY) else jump["destinations"][0] if is_state(MOVING) else m_pos
